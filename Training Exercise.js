...
import {
  LayoutDashboard, Wrench, Store, Calendar, CheckCircle, Package, Users,
  ClipboardList, TrendingUp, Handshake, MapPin, MessageCircle, Truck, AlertCircle,
} from 'lucide-react';
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="Daily Task Completion" value={`${operationsData.dailyTaskCompletion}%`} icon={CheckCircle} />
        <StatCard title="Pending Jobs" value={jobs.filter(j => j.status === 'Pending').length} icon={ClipboardList} />
        <StatCard title="In-Stock Items" value={operationsData.inventoryStatus.inStock} icon={Package} />
        <StatCard title="Out of Stock Items" value={operationsData.inventoryStatus.outOfStock} icon={Truck} />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-md">
          <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
              <AlertCircle className="mr-2 h-5 w-5 text-red-500" /> Top 5 Low Stock Items
          </h3>
          ...
          {jobs.map((job) => (
            <li key={job.id} className="py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center">
              ...
              <div className="flex items-center space-x-2">
                <label htmlFor={`status-select-${job.id}`} className="sr-only">Choose a status</label>
                <select
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard title="Customer Satisfaction" value={`${technicianData.performanceMetrics.customerSatisfaction}/5`} icon={Handshake} />
        <StatCard title="Average Job Time" value={technicianData.performanceMetrics.averageTime} icon={Calendar} />
        <StatCard title="Job Completion Rate" value={`${technicianData.performanceMetrics.completionRate}%`} icon={CheckCircle} />
      </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-md flex flex-col justify-between">
          <div>
            ...
            <div className="flex items-center space-x-4">
              <label htmlFor="status-select" className="sr-only">Choose a status</label>
              <select
                id="status-select"
                value={newStatus}
...
const RetailAssociateDashboard = () => (
  <div className="p-8 space-y-8">
    <h1 className="text-3xl font-bold text-gray-900">Store Dashboard - {retailData.storeName}</h1>
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
      <StatCard title="Daily Sales (Today)" value="+$2,500" icon={TrendingUp} />
      <StatCard title="Upcoming Tasks" value="3" icon={ClipboardList} />
      <StatCard title="Customer Feedback" value="4 new" icon={MessageCircle} />
    </div>
    ...
    <div className="grid lg:grid-cols-2 gap-6">
      ...
      <ChartCard title="Monthly Sales by Category (Last 30 Days)">
        <ResponsiveContainer width="100%" height={300}>
          <BarChart layout="vertical" data={retailData.monthlySalesByCategory}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis type="number" />
